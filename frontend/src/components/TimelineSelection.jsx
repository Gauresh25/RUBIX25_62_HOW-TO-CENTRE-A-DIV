    ...
    <div className="w-full px-4 py-8">
      ...
      {!selectedTimeline ? (
          ...
          {timelines.map((timeline) => (
            ...
            >
              <div className="flex items-center gap-4 mb-4">
                {timeline.historicalCharacter && (
                  <img 
                    src={timeline.historicalCharacter.image} 
                    alt={timeline.historicalCharacter.name}
                    className="w-16 h-16 rounded-full border-2 border-[#8B4513] object-cover"
                  />
                )}
                <div>
                  <h2 className="text-2xl font-koulen text-[#8B4513] mb-2">
                    {timeline.title}
                  </h2>
                  <span className="inline-block bg-[#8B4513] text-white px-3 py-1 rounded text-sm">
                    {timeline.category}
                  </span>
                </div>
              </div>
        ...
        <div className="w-full">
          ...
          <Timeline 
            timelineData={selectedTimeline.timelineData}
            historicalCharacter={selectedTimeline.historicalCharacter}
          />
        </div>
      )}
    </div>
